...
varrun="/run/mosaic-lxc"
varlib="/var/lib"
net_link_key="lxc.net.0.link"
case "$(lxc-info --version)" in [012].*) net_link_key="lxc.network.link" ;; esac
vnic=$(awk "\$1 == \"$net_link_key\" {print \$3}" /var/lib/mosaic/lxc/mosaic/config)
: ${vnic:=mosaic0}

if [ "$vnic" != "mosaic0" ]; then
    echo "vnic is $vnic, bailing out"
    exit 0
else 
    echo "vnic is mosaic0"
fi
...
start() {
    ...
    [ ! -f "${varrun}/network_up" ] || { echo "mosaic-net is already running"; exit 0; }
    ...
    cleanup() {
        set +e
        if [ "$FAILED" = "1" ]; then
            echo "Failed to setup mosaic-net." >&2
            stop force
            exit 1
        fi
    }
...
stop() {
    ...
    [ -f "${varrun}/network_up" ] || [ "$1" = "force" ] || { echo "mosaic-net isn't running"; exit 1; }
